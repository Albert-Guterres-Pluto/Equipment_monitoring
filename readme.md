# 过滤设备监测建模项目

## 项目简介
2026年北京高校数学建模校际联赛 A 题「过滤设备监测」的完整代码实现。  
涵盖数据预处理、透水率规律分析、寿命预测、维护方案优化及敏感性分析。  
运行 `main.py` 可一键生成所有结果和图表。

## 项目结构
项目根目录：EQUIPMENT_MONITORING

  config.py                全局路径与参数（自动基于项目根目录）
  main.py                  一键运行所有模块

  data/                    数据目录
    raw/                   原始附件
      附件1.xlsx
      附件2.xlsx
    processed/             清洗后日频数据
      cleaned_data.csv

  src/                     核心代码
    preprocess.py          数据预处理（前向填充、维护标记）
    analysis.py            第一问：衰减率、季节效应、维护增益
    model.py               第二问：寿命预测模型
    optimize.py            第三、四问：维护优化与敏感性分析

  utils/                   工具脚本
    plot.py                论文图表生成

  results/                 输出结果
    figures/               5张论文用图
      time_series_A1.png
      seasonal.png
      life_decay.png
      optimal_thresholds.png
      sensitivity_heatmap.png
    decay_rates.csv        各设备自然衰减率
    indicators.csv         第一问关键指标汇总
    life_predictions_final.csv  第二问寿命预测结果
    optimal_maintenance.csv    第三问最优维护方案
    seasonal_effect.csv    季节效应（月均偏离）
    sensitivity_analysis.csv   第四问成本波动敏感性

## 环境配置（队友必读）
1. **安装 Python 3.9 或以上版本**  
   推荐 [Anaconda](https://www.anaconda.com) 或从 [python.org](https://www.python.org) 下载。
2. **安装依赖库**  
   打开终端，进入项目根目录，执行：
   ```bash
   pip install pandas numpy scipy openpyxl matplotlib
   ```
   如果使用 Anaconda，也可用：
   ```bash
   conda install pandas numpy scipy openpyxl matplotlib
   ```
3. **确认数据文件**  
   确保 `data/raw/` 内包含 `附件1.xlsx` 和 `附件2.xlsx`，否则程序会报错。

## 如何运行
- 在终端的 **项目根目录** 下执行：
  ```bash
  python main.py
  ```
- 等待约 15 秒，所有结果（CSV 和图片）将自动更新在 `results/` 目录下。
- **注意**：不要进入 `src/` 或 `utils/` 目录执行脚本，务必保持在根目录。

## 路径兼容性说明
- 本项目所有路径均基于 **项目根目录自动定位**，无需手动修改任何配置。
- `config.py` 中通过 `Path(__file__).resolve().parent` 确定根目录，其他脚本均使用 `config.PROJECT_ROOT` 构建绝对路径。
- **因此，只要保持整个文件夹结构完整，无论在谁的电脑上、磁盘位置如何，均可直接运行。**

## 各模块功能与输出
| 模块 | 脚本 | 主要输出 | 关键结论 |
|------|------|----------|----------|
| 数据预处理 | `preprocess.py` | `cleaned_data.csv` | 日频前向填充，标定维护事件 |
| 第一问：规律分析 | `analysis.py` | `decay_rates.csv`, `indicators.csv`, `seasonal_effect.csv` | 平均月衰减 -11.11，中维护增益 19.53，大维护增益 16.58，季节波动幅度 31.21 |
| 第二问：寿命预测 | `model.py` | `life_predictions_final.csv` | 10台过滤器总寿命 5.15~7.46 年 |
| 第三问：维护优化 | `optimize.py` | `optimal_maintenance.csv` | 最优策略仅需中维护，阈值动态42~96 |
| 第四问：敏感性分析 | `optimize.py`（同文件） | `sensitivity_analysis.csv` | 设备价格和维护成本波动时策略稳定 |
| 图表生成 | `utils/plot.py` | `figures/` 下5张PNG | 可直接插入论文 |

## 常见问题排查
- **报错 `ModuleNotFoundError`**：请检查是否安装依赖，手动安装上述库。
- **报错找不到文件**：确认 `data/raw/` 中有 `附件1.xlsx` 和 `附件2.xlsx`，且终端工作目录为项目根目录。
- **图表中文显示为方块**：字体问题，可在 `plot.py` 中删除 `plt.rcParams['font.sans-serif']` 行，或安装 SimHei 字体。
- **运行 `main.py` 后部分步骤跳过**：查看控制台错误信息，多半是路径或数据缺失。

## 论文写作参考
- **建模手**：需复核损伤参数（`DAMAGE_MED = 0.002`, `DAMAGE_LARGE = 0.005`），并在论文中解释大维护未被触发的原因，或讨论引入强制大维护的必要性。
- **论文手**：直接使用 `results/` 下的表格数据和 `figures/` 下的图片，建议论文结构包括：
  - 摘要
  - 问题重述
  - 模型假设与符号说明
  - 数据预处理
  - 问题一：规律分析
  - 问题二：寿命预测
  - 问题三：维护优化
  - 问题四：敏感性分析
  - 模型评价与改进

## 📌 重要更新与讨论（2026-04-25）
### 1. 题目勘误：大维护次数修改
原题中“每年会对每个过滤器进行1至4次不等的大维护”已更正为 **“0至4次”**。  
对应修改：
- `model.py` 中 `get_future_schedule` 函数已调整：若某台设备的历史大维护次数为 0，则未来不再安排大维护；仅有1次历史记录时，默认间隔365天；有2次及以上时取中位数间隔。  
- 此修改已影响部分设备的寿命预测结果（例如 A_8 从 5.57 年降至 5.43 年，A_4 从 7.35 年降至 7.22 年），详见 `results/life_predictions_final.csv`。

### 2. 大维护在优化中为何全部为 0 次？
运行第三问优化后，所有设备的最优策略均 **不包含大维护**（大维护次数 = 0）。  
**这不是代码错误，而是当前参数下的合理数学结果**，原因如下：
- **成本效益不对等**：中维护成本 3 万元，平均增益 19.53；大维护成本 12 万元，平均增益仅 16.58。中维护更便宜且增益更高，算法自然回避大维护。
- **损伤惩罚差异**：每次大维护会使日均衰减率额外增加 0.005（中维护为 0.002），进一步削弱大维护的竞争力。
- **物理含义存疑**：从实际经验看，大维护应该带来更大透水率提升，但第一问基于真实数据的统计未能体现这一点。可能原因：大维护常在透水率很低时进行，恢复空间有限；或样本量少导致统计偏差。

**团队讨论要点**：
- 题目给出 0~4 次的上限，暗示大维护在某些情况下是有益的。当前的 0 次结果可能意味着模型低估了大维护的实际价值。
- 若要更贴合题意，可考虑：
  1. **重新标定大维护增益**（例如取维护后 30 天平均透水率与维护前的差值，或人工设定合理增益 ≥ 25）。
  2. **在优化中加入“强制大维护周期”**（如每年至少 1 次），作为对比方案展示成本变化。
  3. **在第四问敏感性分析中增加“大维护增益波动”场景**，观察增益增大时大维护能否被触发，以此说明参数敏感性。

### 3. 后续行动
- 当前代码与结果可直接用于论文，但需要在论文中充分讨论大维护为 0 的原因、模型的局限及可能的改进方向。
- 若决定调整模型参数或增加对比实验，请联系编程手修改后重新运行 `main.py`。

## 后续优化计划
原计划在期中考试后添加模型评估指标、对比不同算法等，现已记录在案。  
大维护相关改进将根据团队论文需要，在考试后集中讨论并实施。

如有参数或图表需要调整，直接修改对应脚本后重新运行 `main.py` 即可刷新全部结果。
```

给你整理成**可直接粘进 README 的 Markdown 代码块版本**，结构已经排好，直接复制就行：



````
# 📌 第一问建模升级（2026-04-29）

## 🔧 升级背景

原始分析方法主要基于：

- 维护前后单点透水率差值（增益 G）
- 设备长期平均衰减率

该方法存在一个关键缺陷：

> ❗ **仅刻画了维护的短期效果，忽略了对后续性能演化的影响**

这会导致：

- 对不同维护策略的评价偏差
- 难以解释“大维护效果偏弱”等现象
- 影响后续寿命预测与优化模型的合理性

---

## 🧠 升级核心：引入“维护损伤指标 Δd”

为刻画维护对长期性能的影响，本文提出：

> 👉 **维护 = 短期增益（G） + 长期影响（Δd）**

---

## 📐 指标定义

### 1. 维护增益 G（改进版）

由原“单点差值”改为：

- 维护前后窗口（3天）平均透水率差

```text
G = mean(after_window) - mean(before_window)
````

👉 优点：

* 降低测量噪声影响
* 更稳定反映恢复效果

---

### 2. 维护损伤 Δd（新增核心指标）

定义为：

```text
Δd = d_after - d_before
```

其中：

* d_before：维护前窗口斜率（衰减率）
* d_after：维护后窗口斜率

---

## 📊 物理含义

| Δd     | 含义            |
| ------ | ------------- |
| Δd < 0 | 衰减加快 → 设备性能受损 |
| Δd > 0 | 衰减减缓 → 状态得到改善 |

---

## 🔍 数据分析结果（核心发现）

| 指标     | 中维护    | 大维护    |
| ------ | ------ | ------ |
| 平均增益 G | 19.14  | 16.15  |
| 损伤 Δd  | -0.159 | +0.470 |

---

## 🔥 关键结论

### 1️⃣ 中维护

* ✔ 短期恢复能力强（G大）
* ❗ 但会加速后续衰减（Δd < 0）

👉 属于“**短期优化行为**”

---

### 2️⃣ 大维护

* ❗ 短期增益较低（G小）
* ✔ 显著降低衰减速率（Δd > 0）

👉 属于“**系统性修复行为**”

---

## 💥 重要认知升级

> ❗ **单纯使用“增益 G”评价维护效果是不充分的**

引入 Δd 后可以发现：

* 不同维护方式在“短期收益”和“长期性能”之间存在权衡
* 大维护的价值体现在长期稳定性，而非瞬时恢复

---

## 🔗 对后续建模的影响

### ▶ 第二问（寿命预测）

* Δd 可用于修正衰减率
* 提高寿命预测的动态性与真实性

---

### ▶ 第三问（维护优化）

该分析解释了一个关键现象：

> 最优策略中“大维护为0”的结果

原因在于：

* 优化模型原本仅考虑短期收益与成本
* 未显式考虑 Δd 所代表的长期效应

---

👉 因此在优化中：

* 引入约束（如每年至少1次大维护）
* 或加入长期性能指标

以增强工程合理性

---

## 📈 升级效果总结

| 维度    | 升级前   | 升级后         |
| ----- | ----- | ----------- |
| 模型类型  | 描述性统计 | 机制建模        |
| 维护评价  | 单一指标  | 双指标（G + Δd） |
| 解释能力  | 一般    | ⭐⭐⭐⭐⭐       |
| 工程合理性 | 存疑    | 显著提升        |

---

## 🎯 一句话总结（论文/答辩可用）

> 本文将维护效应分解为短期增益与长期影响两个部分，通过引入维护前后衰减率变化 Δd，对不同维护策略的长期性能影响进行了量化，从而提高了模型的解释能力与工程合理性。

---

## 🚀 补充说明

本次升级仅对分析模块进行局部重构，不改变整体工程结构，实现了模型能力与代码稳定性的兼顾。


